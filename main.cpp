...
int main(int argc, char* argv[])  {
    ...
    std::thread threads[MAINCASE];  //will create three threads
    ...
    CmdLineArgs *ptrCLA = new CmdLineArgs(thread); //Instantiate a class named CmdLineArgs. It is here where we define the default values and the real-time values of the command line arguments.
    ...
    const unsigned short numberOfPossibleCommandLineArguments = sizeof(longopts);
...
//        ptrbc2 ->~BaseClass();
    ...
    if (thread) {
        ...
        cout << "Last to Join: Time to execute the M1M2KWHCASE " << ptrSWmain[M1KWHCASE]->ElapsedUs() << " µsec. " << endl;
...
//        ptrbc1 ->~BaseClass();
    ...
    std::cout << "Line " << __LINE__ << " Total Execution Time of " << *(argv + 0) << ", for all cases, is " << testDuration.count() << " µsec." << std::endl;
    for(unsigned short j=0 ; j < numberOfThreads; j++) delete ptrSWmain[j];
//    return rc;
